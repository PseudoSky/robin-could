...
min_diameter = 21.00;  // Diameter of pen changer

//min_diameter = 85.22;  // Diameter of pen changer
bearing = 19.05;  // Bearing space 


// Number of teeth, standard Mendel T5 belt = 8, gives Outside Diameter of 11.88mm
// 30 Teeth sets up for 48mm Pulley with XL

teeth = 40;			

// 1=MXL 2=40DP 3=XL 4=H 5=T2.5 6=T5 7=T10 8=AT5 9=HTD_3mm 10=HTD_5mm 11=HTD_8mm 12=GT2_2mm 13=GT2_3mm 14=GT2_5mm
profile = 3;		


//motor_shaft = 19.05;  // Bearing is 19.05 mm
...
teeth = tooth_count( min_diameter, 5.08, 0.254 );
diam = tooth_spacing(5.08,0.254);
echo(str("\n\n",teeth," Teeth\n","\nDia: ", diam ,"mm\n\n"));
...
function tooth_count(dia,tooth_pitch,pitch_line_offset)
	= ceil((3.14159265*(2*pitch_line_offset+dia))/tooth_pitch) ;

pulley_db = [
	["MXL" , MXL_pulley_dia , 0.508 , 1.321 ] ,
	["40 D.P." , 40DP_pulley_dia , 0.457 , 1.226 ] ,
	["XL" , XL_pulley_dia , 1.27, 3.051 ] ,
	["H" , H_pulley_dia ,1.905 , 5.359 ] ,
	["T2.5" , T2_5_pulley_dia , 0.7 , 1.678 ] ,
	["T5" , T5_pulley_dia , 1.19 , 3.264 ] ,
	["T10" , T10_pulley_dia , 2.5 , 6.13 ] ,
	["AT5" , AT5_pulley_dia , 1.19 , 4.268 ] ,
	["HTD 3mm" , HTD_3mm_pulley_dia , 1.289 , 2.27 ] ,
	["HTD 5mm" , HTD_5mm_pulley_dia , 2.199 , 3.781 ] ,
	["HTD 8mm" , HTD_8mm_pulley_dia , 3.607 , 6.603 ] ,
	["GT2 2mm" , GT2_2mm_pulley_dia , 0.764 , 1.494 ] ,
	["GT2 3mm" , GT2_3mm_pulley_dia , 1.169 , 2.31 ] ,
	["GT2 5mm" , GT2_5mm_pulley_dia , 1.969 , 3.952 ] 
];
...
module pull_for( pulley_name,tx=0, tz=-10,attrs=[] ){
    idx=0;
    echo(tx);
    tx=tx*diam+30;
    is_profile=(str(search([pulley_name],[1,2,3,4,5,6,7,8,9,0]))!="[[]]");
    echo(is_profile);
    if(is_profile) {
        pulley_name=pulley_lookup[search([pulley_name],pulley_lookup)[0]][1];
        echo(pulley_name);
        idx=pulley_lookup[search([pulley_name],pulley_lookup)[0]][1]-1;
        echo(pulley_db[idx]);
        translate([tx,0,tz]) pulley ( pulley_db[idx][0],pulley_db[idx][1],pulley_db[idx][2],pulley_db[idx][3]);
    }else{
        idx=search([pulley_name],pulley_lookup)[0]-1;
        echo(pulley_db[idx]);
        projection(cut=true) translate([tx,0,tz]) pulley ( pulley_db[idx][0],pulley_db[idx][1],pulley_db[idx][2],pulley_db[idx][3]);
...
// Belt retainer above teeth, 0 = No, 1 = Yes
retainer = 1;
...
// if ( profile == 1 ) { projection(cut=true) translate([0,0,-10]) pulley ( "MXL" , MXL_pulley_dia , 0.508 , 1.321 ); }
// if ( profile == 2 ) { projection(cut=true) translate([0,0,-10]) pulley ( "40 D.P." , 40DP_pulley_dia , 0.457 , 1.226 ); }
// if ( profile == 3 ) { projection(cut=true) translate([0,0,-10]) pulley ( "XL" , XL_pulley_dia , 1.27, 3.051 ); }
// if ( profile == 4 ) { projection(cut=true) translate([0,0,-10]) pulley ( "H" , H_pulley_dia ,1.905 , 5.359 ); }
// if ( profile == 5 ) { projection(cut=true) translate([0,0,-10]) pulley ( "T2.5" , T2_5_pulley_dia , 0.7 , 1.678 ); }
// if ( profile == 6 ) { projection(cut=true) translate([0,0,-10]) pulley ( "T5" , T5_pulley_dia , 1.19 , 3.264 ); }
// if ( profile == 7 ) { projection(cut=true) translate([0,0,-10]) pulley ( "T10" , T10_pulley_dia , 2.5 , 6.13 ); }
// if ( profile == 8 ) { projection(cut=true) translate([0,0,-10]) pulley ( "AT5" , AT5_pulley_dia , 1.19 , 4.268 ); }
// if ( profile == 9 ) { projection(cut=true) translate([0,0,-10]) pulley ( "HTD 3mm" , HTD_3mm_pulley_dia , 1.289 , 2.27 ); }
// if ( profile == 10 ) { projection(cut=true) translate([0,0,-10]) pulley ( "HTD 5mm" , HTD_5mm_pulley_dia , 2.199 , 3.781 ); }
// if ( profile == 11 ) { projection(cut=true) translate([0,0,-10]) pulley ( "HTD 8mm" , HTD_8mm_pulley_dia , 3.607 , 6.603 ); }
// if ( profile == 12 ) { projection(cut=true) translate([0,0,-10]) pulley ( "GT2 2mm" , GT2_2mm_pulley_dia , 0.764 , 1.494 ); }
// if ( profile == 13 ) { projection(cut=true) translate([0,0,-10]) pulley ( "GT2 3mm" , GT2_3mm_pulley_dia , 1.169 , 2.31 ); }
// if ( profile == 14 ) { projection(cut=true) translate([0,0,-10]) pulley ( "GT2 5mm" , GT2_5mm_pulley_dia , 1.969 , 3.952 ); }
